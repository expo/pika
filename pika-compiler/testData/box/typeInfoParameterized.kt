...
fun box(): String {
  ...
  if (listStringInfo.parameters.size != 1) return "FAIL: List<String> should have 1 type argument"

  val listArg = listStringInfo.parameters[0] as? PTypeDescriptor.Concrete
  ...
  if (mapInfo.parameters.size != 2) return "FAIL: Map<String, Int> should have 2 type arguments"

  val mapKeyArg = mapInfo.parameters[0] as? PTypeDescriptor.Concrete
    ?: return "FAIL: Map key should be Concrete"
  if (mapKeyArg.pType.jClass != String::class.java) return "FAIL: Map key kClass"

  val mapValueArg = mapInfo.parameters[1] as? PTypeDescriptor.Concrete
