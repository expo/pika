...
fun box(): String {
  ...
  val personData = personDescriptor.introspection
    ?: return "FAIL: Person introspection should not be null"
  ...
  // The type descriptor of the address property should carry Address's introspection
  ...
  val addressData = addressTypeDescriptor.introspection
    ?: return "FAIL: address property type introspection should not be null"

  if (addressData.jClass != Address::class.java)
    return "FAIL: nested introspection.jClass should be Address"
  ...
  val cityProp = addressData.properties.find { it.name == "city" }
    ?: return "FAIL: city property not found in nested introspection"

  // Cross-check: nested introspection matches introspectionOf<Address>()
  ...
  // name property type (String) should have null introspection
  ...
  if (nameType.introspection != null)
    return "FAIL: String property type introspection should be null"
