package io.github.expo.pika

/**
 * Returns a type descriptor for the specified type parameter T.
 * The compiler plugin replaces calls to this function at compile time.
 */
public fun <T> typeDescriptorOf(): PTypeDescriptor =
  throw NotImplementedError("typeDescriptorOf<T>() should be replaced by the compiler plugin")

/**
 * Internal function called when typeDescriptorOf<T>() is used with a non-reified type parameter.
 * This provides a clear error message at runtime.
 * Returns PTypeDescriptor for bytecode verifier compatibility (but always throws).
 */
@PublishedApi
internal fun throwNonReifiedTypeDescriptorError(): PTypeDescriptor =
  throw IllegalStateException(
    "typeDescriptorOf<T>() requires a reified type parameter. " +
      "Use 'inline fun <reified T>' or call typeDescriptorOf<T>() with a concrete type."
  )

